...
use crate::AppState;
use crate::repository::unit_repository;
use crate::models::unit::ModifiedUnit;

pub async fn find(data: web::Data<AppState>) -> Result<HttpResponse> {
    let conn = data.pool.get().expect("failed to get connection");
    let result = web::block(move || unit_repository::find(&conn))
        .await;
...
pub async fn get(data: web::Data<AppState>, path: web::Path<(i32,)>) -> Result<HttpResponse> {
    let conn = data.pool.get().expect("failed to get connection");
    let result = web::block(move || unit_repository::get(&conn, path.into_inner().0))
        .await;
...
pub async fn create(data: web::Data<AppState>,
                    role: web::Json<ModifiedUnit>) -> Result<HttpResponse> {
    let conn = data.pool.get().expect("failed to get connection");
    let result = web::block(move || unit_repository::create(&conn, role.0))
        .await;
...
pub async fn update(data: web::Data<AppState>,
                    role: web::Json<ModifiedUnit>,
                    path: web::Path<(i32,)>) -> Result<HttpResponse> {
    let conn = data.pool.get().expect("failed to get connection");
    let result = web::block(move || unit_repository::update(&conn, path.into_inner().0, role.0))
        .await;
...
pub async fn delete(data: web::Data<AppState>,
                    path: web::Path<(i32,)>) -> Result<HttpResponse> {
    let conn = data.pool.get().expect("failed to get connection");
    let result = web::block(move || unit_repository::delete(&conn, path.into_inner().0))
        .await;
